...
d3Array){function numbers(values,valueof){var index,$jscomp$iter$107,$jscomp$key$value,value$430,$jscomp$iter$106,value;return $jscomp.generator.createGenerator(numbers,function($jscomp$generator$context){switch($jscomp$generator$context.nextAddress){case 1:if(null==valueof){$jscomp$generator$context.jumpTo(2);break}index=-1;$jscomp$iter$107=$jscomp.makeIterator(values);$jscomp$key$value=$jscomp$iter$107.next();case 4:if($jscomp$key$value.done){$jscomp$generator$context.jumpTo(0);break}value$430=
$jscomp$key$value.value;value$430=valueof(value$430,++index,values);if(!(null!=value$430&&""!==value$430&&(value$430=+value$430)>=value$430)){$jscomp$generator$context.jumpTo(5);break}return $jscomp$generator$context.yield(value$430,5);case 5:$jscomp$key$value=$jscomp$iter$107.next();$jscomp$generator$context.jumpTo(4);break;case 2:$jscomp$iter$106=$jscomp.makeIterator(values),$jscomp$key$value=$jscomp$iter$106.next();case 9:if($jscomp$key$value.done){$jscomp$generator$context.jumpTo(0);break}value=
...
y){var X=0,Y=0,XY=0,X2=0,n=0;visitPoints(data,x$jscomp$0,y,function(dx,dy){++n;X+=(dx-X)/n;Y+=(dy-Y)/n;XY+=(dx*dy-XY)/n;X2+=(dx*dx-X2)/n});var coef=ols(X,Y,XY,X2),predict=function(x){return coef[0]+coef[1]*x};return{coef:coef,predict:predict,rSquared:rSquared(data,x$jscomp$0,y,Y,predict)}}function quad(data,x$jscomp$0,y){var $jscomp$destructuring$var32=$jscomp.makeIterator(points(data,x$jscomp$0,y)),xv=$jscomp$destructuring$var32.next().value,yv=$jscomp$destructuring$var32.next().value,ux=$jscomp$destructuring$var32.next().value,
uy=$jscomp$destructuring$var32.next().value,n=xv.length,X3=$jscomp$destructuring$var32=0,X4=0,XY=0,X2Y=0,i;for(i=0;i<n;){var dx=xv[i];var dy=yv[i++];var x2=dx*dx;$jscomp$destructuring$var32+=(x2-$jscomp$destructuring$var32)/i;X3+=(x2*dx-X3)/i;X4+=(x2*x2-X4)/i;XY+=(dx*dy-XY)/i;X2Y+=(x2*dy-X2Y)/i}xv=X4-$jscomp$destructuring$var32*$jscomp$destructuring$var32;yv=$jscomp$destructuring$var32*xv-X3*X3;var a=(X2Y*$jscomp$destructuring$var32-XY*X3)/yv,b=(XY*xv-X2Y*X3)/yv,c=-a*$jscomp$destructuring$var32;$jscomp$destructuring$var32=
function(x){x-=ux;return a*x*x+b*x+c+uy};return{coef:[c-b*ux+a*ux*ux+uy,b-2*a*ux,a],predict:$jscomp$destructuring$var32,rSquared:rSquared(data,x$jscomp$0,y,uy,$jscomp$destructuring$var32)}}function uncenter(k,a,x,y){var z=Array(k),i;for(i=0;i<k;++i)z[i]=0;for(i=k-1;0<=i;--i){var v=a[i];var c=1;z[i]+=v;for(k=1;k<=i;++k)c*=(i+1-k)/k,z[i-k]+=v*Math.pow(x,k)*c}z[0]+=y;return z}function gaussianElimination(matrix){var n=matrix.length-1,coef=[],i,j,k;for(i=0;i<n;++i){var r=i;for(j=i+1;j<n;++j)Math.abs(matrix[i][j])>
...
return{coef:coef,predict:predict,rSquared:rSquared(data,x$jscomp$0,y,Y,predict)}};exports.regressionPoly=function(data,x$jscomp$0,y$jscomp$0,order){if(1===order)return linear(data,x$jscomp$0,y$jscomp$0);if(2===order)return quad(data,x$jscomp$0,y$jscomp$0);var $jscomp$destructuring$var33=$jscomp.makeIterator(points(data,x$jscomp$0,y$jscomp$0)),xv=$jscomp$destructuring$var33.next().value,yv=$jscomp$destructuring$var33.next().value,ux=$jscomp$destructuring$var33.next().value,uy=$jscomp$destructuring$var33.next().value;
$jscomp$destructuring$var33=xv.length;var lhs=[],rhs=[],k=order+1,i,l,v;for(i=0;i<k;++i){for(v=l=0;l<$jscomp$destructuring$var33;++l)v+=Math.pow(xv[l],i)*yv[l];lhs.push(v);var c=new Float64Array(k);for(order=0;order<k;++order){for(v=l=0;l<$jscomp$destructuring$var33;++l)v+=Math.pow(xv[l],i+order);c[order]=v}rhs.push(c)}rhs.push(lhs);var coef=gaussianElimination(rhs);xv=function(x){x-=ux;var y=uy+coef[0]+coef[1]*x+coef[2]*x*x;for(i=3;i<k;++i)y+=coef[i]*Math.pow(x,i);return y};return{coef:uncenter(k,
