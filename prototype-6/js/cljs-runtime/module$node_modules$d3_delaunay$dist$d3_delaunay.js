...
dists[ids[right]]&&swap(ids,left,right);dists[ids[i]]>dists[ids[right]]&&swap(ids,i,right);dists[ids[left]]>dists[ids[i]]&&swap(ids,left,i);tempDist=ids[i];for(j=dists[tempDist];;){do i++;while(dists[ids[i]]<j);do temp--;while(dists[ids[temp]]>j);if(temp<i)break;swap(ids,i,temp)}ids[left+1]=ids[temp];ids[temp]=tempDist;right-i+1>=temp-left?(quicksort(ids,dists,i,right),quicksort(ids,dists,left,temp-1)):(quicksort(ids,dists,left,temp-1),quicksort(ids,dists,i,right))}}function swap(arr,i,j){var tmp=
arr[i];arr[i]=arr[j];arr[j]=tmp}function defaultGetX(p){return p[0]}function defaultGetY(p){return p[1]}function pointX(p){return p[0]}function pointY(p){return p[1]}function collinear$jscomp$0(d){var triangles=d.triangles;d=d.coords;for(var i=0;i<triangles.length;i+=3){var a=2*triangles[i],b=2*triangles[i+1],c=2*triangles[i+2];if(1E-10<(d[c]-d[a])*(d[b+1]-d[a+1])-(d[b]-d[a])*(d[c+1]-d[a+1]))return!1}return!0}function jitter(x,y,r){return[x+Math.sin(x+y)*r,y+Math.cos(x-y)*r]}function flatIterable(points,
fx,fy,that){var i,$jscomp$iter$119,$jscomp$key$p,p;return $jscomp.generator.createGenerator(flatIterable,function($jscomp$generator$context){switch($jscomp$generator$context.nextAddress){case 1:i=0,$jscomp$iter$119=$jscomp.makeIterator(points),$jscomp$key$p=$jscomp$iter$119.next();case 2:if($jscomp$key$p.done){$jscomp$generator$context.jumpTo(0);break}p=$jscomp$key$p.value;return $jscomp$generator$context.yield(fx.call(that,p,i,points),5);case 5:return $jscomp$generator$context.yield(fy.call(that,
p,i,points),6);case 6:++i,$jscomp$key$p=$jscomp$iter$119.next(),$jscomp$generator$context.jumpTo(2)}})}var EPSILON=Math.pow(2,-52),EDGE_STACK=new Uint32Array(512),Delaunator=function(coords){var n=coords.length>>1;if(0<n&&"number"!==typeof coords[0])throw Error("Expected coords to contain numbers.");this.coords=coords;coords=Math.max(2*n-5,0);this._triangles=new Uint32Array(3*coords);this._halfedges=new Int32Array(3*coords);this._hashSize=Math.ceil(Math.sqrt(n));this._hullPrev=new Uint32Array(n);
this._hullNext=new Uint32Array(n);this._hullTri=new Uint32Array(n);this._hullHash=(new Int32Array(this._hashSize)).fill(-1);this._ids=new Uint32Array(n);this._dists=new Float64Array(n);this.update()};Delaunator.from=function(points,getX,getY){getX=void 0===getX?defaultGetX:getX;getY=void 0===getY?defaultGetY:getY;for(var n=points.length,coords=new Float64Array(2*n),i=0;i<n;i++){var p=points[i];coords[2*i]=getX(p);coords[2*i+1]=getY(p)}return new Delaunator(coords)};Delaunator.prototype.update=function(){for(var JSCompiler_object_inline_x_4288,
JSCompiler_object_inline_y_4289,coords=this.coords,hullPrev=this._hullPrev,hullNext=this._hullNext,hullTri=this._hullTri,hullHash=this._hullHash,n=coords.length>>1,minX=Infinity,minY=Infinity,maxX=-Infinity,maxY=-Infinity,i=0;i<n;i++){var x=coords[2*i];JSCompiler_object_inline_x_4288=coords[2*i+1];x<minX&&(minX=x);JSCompiler_object_inline_x_4288<minY&&(minY=JSCompiler_object_inline_x_4288);x>maxX&&(maxX=x);JSCompiler_object_inline_x_4288>maxY&&(maxY=JSCompiler_object_inline_x_4288);this._ids[i]=i}maxX=
(minX+maxX)/2;minY=(minY+maxY)/2;minX=Infinity;for(maxY=0;maxY<n;maxY++)if(i=dist(maxX,minY,coords[2*maxY],coords[2*maxY+1]),i<minX){var i0=maxY;minX=i}minY=coords[2*i0];maxY=coords[2*i0+1];minX=Infinity;for(maxX=0;maxX<n;maxX++)if(maxX!==i0&&(i=dist(minY,maxY,coords[2*maxX],coords[2*maxX+1]),i<minX&&0<i)){var i1=maxX;minX=i}minX=coords[2*i1];maxX=coords[2*i1+1];JSCompiler_object_inline_x_4288=Infinity;for(i=0;i<n;i++)if(i!==i0&&i!==i1){x=minX-minY;var dy=maxX-maxY;JSCompiler_object_inline_y_4289=
coords[2*i]-minY;var ey=coords[2*i+1]-maxY,bl=x*x+dy*dy,cl=JSCompiler_object_inline_y_4289*JSCompiler_object_inline_y_4289+ey*ey,d=.5/(x*ey-dy*JSCompiler_object_inline_y_4289);dy=(ey*bl-dy*cl)*d;x=(x*cl-JSCompiler_object_inline_y_4289*bl)*d;x=dy*dy+x*x;if(x<JSCompiler_object_inline_x_4288){var i2=i;JSCompiler_object_inline_x_4288=x}}i=coords[2*i2];x=coords[2*i2+1];if(Infinity===JSCompiler_object_inline_x_4288){for(hullNext=0;hullNext<n;hullNext++)this._dists[hullNext]=coords[2*hullNext]-coords[0]||
coords[2*hullNext+1]-coords[1];quicksort(this._ids,this._dists,0,n-1);hullNext=new Uint32Array(n);hullPrev=coords=0;for(hullTri=-Infinity;hullPrev<n;hullPrev++)hullHash=this._ids[hullPrev],this._dists[hullHash]>hullTri&&(hullNext[coords++]=hullHash,hullTri=this._dists[hullHash]);this.hull=hullNext.subarray(0,coords);this.triangles=new Uint32Array(0);this.halfedges=new Uint32Array(0)}else{orient(minY,maxY,minX,maxX,i,x)&&(JSCompiler_object_inline_x_4288=i1,JSCompiler_object_inline_y_4289=minX,bl=maxX,
i1=i2,minX=i,maxX=x,i2=JSCompiler_object_inline_x_4288,i=JSCompiler_object_inline_y_4289,x=bl);JSCompiler_object_inline_y_4289=minX-minY;JSCompiler_object_inline_x_4288=maxX-maxY;bl=i-minY;ey=x-maxY;cl=JSCompiler_object_inline_y_4289*JSCompiler_object_inline_y_4289+JSCompiler_object_inline_x_4288*JSCompiler_object_inline_x_4288;d=bl*bl+ey*ey;dy=.5/(JSCompiler_object_inline_y_4289*ey-JSCompiler_object_inline_x_4288*bl);JSCompiler_object_inline_x_4288=minY+(ey*cl-JSCompiler_object_inline_x_4288*d)*
dy;JSCompiler_object_inline_y_4289=maxY+(JSCompiler_object_inline_y_4289*d-bl*cl)*dy;this._cx=JSCompiler_object_inline_x_4288;this._cy=JSCompiler_object_inline_y_4289;for(bl=0;bl<n;bl++)this._dists[bl]=dist(coords[2*bl],coords[2*bl+1],JSCompiler_object_inline_x_4288,JSCompiler_object_inline_y_4289);quicksort(this._ids,this._dists,0,n-1);this._hullStart=i0;n=3;hullNext[i0]=hullPrev[i2]=i1;hullNext[i1]=hullPrev[i0]=i2;hullNext[i2]=hullPrev[i1]=i0;hullTri[i0]=0;hullTri[i1]=1;hullTri[i2]=2;hullHash.fill(-1);
hullHash[this._hashKey(minY,maxY)]=i0;hullHash[this._hashKey(minX,maxX)]=i1;hullHash[this._hashKey(i,x)]=i2;this.trianglesLen=0;this._addTriangle(i0,i1,i2,-1,-1,-1);minY=0;for(minX=maxY=void 0;minY<this._ids.length;minY++)if(maxX=this._ids[minY],i=coords[2*maxX],x=coords[2*maxX+1],!(0<minY&&Math.abs(i-maxY)<=EPSILON&&Math.abs(x-minX)<=EPSILON)&&(maxY=i,minX=x,maxX!==i0&&maxX!==i1&&maxX!==i2)){JSCompiler_object_inline_y_4289=JSCompiler_object_inline_x_4288=0;for(bl=this._hashKey(i,x);JSCompiler_object_inline_y_4289<
this._hashSize&&(JSCompiler_object_inline_x_4288=hullHash[(bl+JSCompiler_object_inline_y_4289)%this._hashSize],-1===JSCompiler_object_inline_x_4288||JSCompiler_object_inline_x_4288===hullNext[JSCompiler_object_inline_x_4288]);JSCompiler_object_inline_y_4289++);JSCompiler_object_inline_y_4289=JSCompiler_object_inline_x_4288=hullPrev[JSCompiler_object_inline_x_4288];for(bl=void 0;bl=hullNext[JSCompiler_object_inline_y_4289],!orient(i,x,coords[2*JSCompiler_object_inline_y_4289],coords[2*JSCompiler_object_inline_y_4289+
1],coords[2*bl],coords[2*bl+1]);)if(JSCompiler_object_inline_y_4289=bl,JSCompiler_object_inline_y_4289===JSCompiler_object_inline_x_4288){JSCompiler_object_inline_y_4289=-1;break}if(-1!==JSCompiler_object_inline_y_4289){cl=this._addTriangle(JSCompiler_object_inline_y_4289,maxX,hullNext[JSCompiler_object_inline_y_4289],-1,-1,hullTri[JSCompiler_object_inline_y_4289]);hullTri[maxX]=this._legalize(cl+2);hullTri[JSCompiler_object_inline_y_4289]=cl;n++;for(d=hullNext[JSCompiler_object_inline_y_4289];bl=
hullNext[d],orient(i,x,coords[2*d],coords[2*d+1],coords[2*bl],coords[2*bl+1]);)cl=this._addTriangle(d,maxX,bl,hullTri[maxX],-1,hullTri[d]),hullTri[maxX]=this._legalize(cl+2),hullNext[d]=d,n--,d=bl;if(JSCompiler_object_inline_y_4289===JSCompiler_object_inline_x_4288)for(;bl=hullPrev[JSCompiler_object_inline_y_4289],orient(i,x,coords[2*bl],coords[2*bl+1],coords[2*JSCompiler_object_inline_y_4289],coords[2*JSCompiler_object_inline_y_4289+1]);)cl=this._addTriangle(bl,maxX,JSCompiler_object_inline_y_4289,
-1,hullTri[JSCompiler_object_inline_y_4289],hullTri[bl]),this._legalize(cl+2),hullTri[bl]=cl,hullNext[JSCompiler_object_inline_y_4289]=JSCompiler_object_inline_y_4289,n--,JSCompiler_object_inline_y_4289=bl;this._hullStart=hullPrev[maxX]=JSCompiler_object_inline_y_4289;hullNext[JSCompiler_object_inline_y_4289]=hullPrev[d]=maxX;hullNext[maxX]=d;hullHash[this._hashKey(i,x)]=maxX;hullHash[this._hashKey(coords[2*JSCompiler_object_inline_y_4289],coords[2*JSCompiler_object_inline_y_4289+1])]=JSCompiler_object_inline_y_4289}}this.hull=
new Uint32Array(n);coords=0;for(hullPrev=this._hullStart;coords<n;coords++)this.hull[coords]=hullPrev,hullPrev=hullNext[hullPrev];this.triangles=this._triangles.subarray(0,this.trianglesLen);this.halfedges=this._halfedges.subarray(0,this.trianglesLen)}};Delaunator.prototype._hashKey=function(x,y){var JSCompiler_temp_const=Math;x-=this._cx;y-=this._cy;x/=Math.abs(x)+Math.abs(y);return JSCompiler_temp_const.floor.call(JSCompiler_temp_const,(0<y?3-x:1+x)/4*this._hashSize)%this._hashSize};Delaunator.prototype._legalize=
...
this._y1=+y)+"h"+ +w+"v"+ +h+"h"+-w+"Z"};Path.prototype.value=function(){return this._||null};var Polygon=function(){this._=[]};Polygon.prototype.moveTo=function(x,y){this._.push([x,y])};Polygon.prototype.closePath=function(){this._.push(this._[0].slice())};Polygon.prototype.lineTo=function(x,y){this._.push([x,y])};Polygon.prototype.value=function(){return this._.length?this._:null};var Voronoi=function(delaunay,$jscomp$destructuring$var65){var $jscomp$destructuring$var66=$jscomp.makeIterator(void 0===
$jscomp$destructuring$var65?[0,0,960,500]:$jscomp$destructuring$var65);$jscomp$destructuring$var65=$jscomp$destructuring$var66.next().value;var ymin=$jscomp$destructuring$var66.next().value,xmax=$jscomp$destructuring$var66.next().value;$jscomp$destructuring$var66=$jscomp$destructuring$var66.next().value;if(!((xmax=+xmax)>=($jscomp$destructuring$var65=+$jscomp$destructuring$var65)&&($jscomp$destructuring$var66=+$jscomp$destructuring$var66)>=(ymin=+ymin)))throw Error("invalid bounds");this.delaunay=
delaunay;this._circumcenters=new Float64Array(2*delaunay.points.length);this.vectors=new Float64Array(2*delaunay.points.length);this.xmax=xmax;this.xmin=$jscomp$destructuring$var65;this.ymax=$jscomp$destructuring$var66;this.ymin=ymin;this._init()};Voronoi.prototype.update=function(){this.delaunay.update();this._init();return this};Voronoi.prototype._init=function(){var $jscomp$destructuring$var68=this.delaunay,points=$jscomp$destructuring$var68.points,hull=$jscomp$destructuring$var68.hull,triangles=
$jscomp$destructuring$var68.triangles;$jscomp$destructuring$var68=this.vectors;for(var circumcenters=this.circumcenters=this._circumcenters.subarray(0,triangles.length/3*2),i=0,j=0,n=triangles.length,x,y;i<n;i+=3,j+=2){y=2*triangles[i];var t2=2*triangles[i+1],t3=2*triangles[i+2];x=points[y];y=points[y+1];var x3=points[t3],y3=points[t3+1];t3=points[t2]-x;var dy=points[t2+1]-y;t2=x3-x;var ey=y3-y,bl=t3*t3+dy*dy,cl=t2*t2+ey*ey,ab=2*(t3*ey-dy*t2);ab?1E-8>Math.abs(ab)?(x=(x+x3)/2,y=(y+y3)/2):(x3=1/ab,
x+=(ey*bl-dy*cl)*x3,y+=(t3*cl-t2*bl)*x3):(x=(x+x3)/2-1E8*ey,y=(y+y3)/2+1E8*t2);circumcenters[j]=x;circumcenters[j+1]=y}triangles=hull[hull.length-1];i=4*triangles;n=points[2*triangles];y=points[2*triangles+1];$jscomp$destructuring$var68.fill(0);for(t3=0;t3<hull.length;++t3)triangles=hull[t3],circumcenters=i,j=n,x=y,i=4*triangles,n=points[2*triangles],y=points[2*triangles+1],$jscomp$destructuring$var68[circumcenters+2]=$jscomp$destructuring$var68[i]=x-y,$jscomp$destructuring$var68[circumcenters+3]=
$jscomp$destructuring$var68[i+1]=n-j};Voronoi.prototype.render=function(context){var buffer=null==context?context=new Path:void 0,$jscomp$destructuring$var70=this.delaunay,halfedges=$jscomp$destructuring$var70.halfedges,inedges=$jscomp$destructuring$var70.inedges;$jscomp$destructuring$var70=$jscomp$destructuring$var70.hull;var circumcenters=this.circumcenters,vectors=this.vectors;if(1>=$jscomp$destructuring$var70.length)return null;for(var i=0,n=halfedges.length;i<n;++i){var j=halfedges[i];if(!(j<
i)){var ti=2*Math.floor(i/3);j=2*Math.floor(j/3);this._renderSegment(circumcenters[ti],circumcenters[ti+1],circumcenters[j],circumcenters[j+1],context)}}halfedges=$jscomp$destructuring$var70[$jscomp$destructuring$var70.length-1];for(i=0;i<$jscomp$destructuring$var70.length;++i)j=halfedges,halfedges=$jscomp$destructuring$var70[i],ti=2*Math.floor(inedges[halfedges]/3),n=circumcenters[ti],ti=circumcenters[ti+1],j*=4,(j=this._project(n,ti,vectors[j+2],vectors[j+3]))&&this._renderSegment(n,ti,j[0],j[1],
context);return buffer&&buffer.value()};Voronoi.prototype.renderBounds=function(context){var buffer=null==context?context=new Path:void 0;context.rect(this.xmin,this.ymin,this.xmax-this.xmin,this.ymax-this.ymin);return buffer&&buffer.value()};Voronoi.prototype.renderCell=function(i,context){var buffer=null==context?context=new Path:void 0;i=this._clip(i);if(null!==i&&i.length){context.moveTo(i[0],i[1]);for(var n=i.length;i[0]===i[n-2]&&i[1]===i[n-1]&&1<n;)n-=2;for(var i$506=2;i$506<n;i$506+=2)i[i$506]===
i[i$506-2]&&i[i$506+1]===i[i$506-1]||context.lineTo(i[i$506],i[i$506+1]);context.closePath();return buffer&&buffer.value()}};Voronoi.prototype.cellPolygons=function $jscomp$generator$function(){var $jscomp$generator$this=this,$jscomp$destructuring$var71,$jscomp$destructuring$var72,points,i,n,cell;return $jscomp.generator.createGenerator($jscomp$generator$function,function($jscomp$generator$context){switch($jscomp$generator$context.nextAddress){case 1:$jscomp$destructuring$var71=$jscomp$generator$this,
$jscomp$destructuring$var72=$jscomp$destructuring$var71.delaunay,points=$jscomp$destructuring$var72.points,i=0,n=points.length/2;case 2:if(!(i<n)){$jscomp$generator$context.jumpTo(0);break}cell=$jscomp$generator$this.cellPolygon(i);if(!cell){$jscomp$generator$context.jumpTo(3);break}cell.index=i;return $jscomp$generator$context.yield(cell,6);case 6:case 3:++i,$jscomp$generator$context.jumpTo(2)}})};Voronoi.prototype.cellPolygon=function(i){var polygon=new Polygon;this.renderCell(i,polygon);return polygon.value()};
...
$jscomp$iter$118,$jscomp$key$j,j,cj,ai,li,aj,lj;return $jscomp.generator.createGenerator($jscomp$generator$function,function($jscomp$generator$context){switch($jscomp$generator$context.nextAddress){case 1:ci=$jscomp$generator$this._clip(i);if(!ci){$jscomp$generator$context.jumpTo(0);break}$jscomp$iter$118=$jscomp.makeIterator($jscomp$generator$this.delaunay.neighbors(i));$jscomp$key$j=$jscomp$iter$118.next();case 3:if($jscomp$key$j.done){$jscomp$generator$context.jumpTo(0);break}j=$jscomp$key$j.value;
...
2;$jscomp$generator$context.jumpTo(10);break;case 4:$jscomp$key$j=$jscomp$iter$118.next(),$jscomp$generator$context.jumpTo(3)}})};Voronoi.prototype._cell=function(i){var circumcenters=this.circumcenters,$jscomp$destructuring$var74=this.delaunay,halfedges=$jscomp$destructuring$var74.halfedges,triangles=$jscomp$destructuring$var74.triangles;$jscomp$destructuring$var74=$jscomp$destructuring$var74.inedges[i];if(-1===$jscomp$destructuring$var74)return null;var points=[],e=$jscomp$destructuring$var74;do{var t=
Math.floor(e/3);points.push(circumcenters[2*t],circumcenters[2*t+1]);e=2===e%3?e-2:e+1;if(triangles[e]!==i)break;e=halfedges[e]}while(e!==$jscomp$destructuring$var74&&-1!==e);return points};Voronoi.prototype._clip=function(i){if(0===i&&1===this.delaunay.hull.length)return[this.xmax,this.ymin,this.xmax,this.ymax,this.xmin,this.ymax,this.xmin,this.ymin];var points=this._cell(i);if(null===points)return null;var V=this.vectors,v=4*i;return V[v]||V[v+1]?this._clipInfinite(i,points,V[v],V[v+1],V[v+2],V[v+
...
points[2*d+1],points[2*f],points[2*f+1]];d=1E-8*Math.hypot(d[3]-d[1],d[2]-d[0]);f=0;for(var n=points.length/2;f<n;++f){var p=jitter(points[2*f],points[2*f+1],d);points[2*f]=p[0];points[2*f+1]=p[1]}this._delaunator=new Delaunator(points)}else delete this.collinear;p=this.halfedges=this._delaunator.halfedges;d=this.hull=this._delaunator.hull;var triangles=this.triangles=this._delaunator.triangles;f=this.inedges.fill(-1);n=this._hullIndex.fill(-1);for(var e$509=0,n$510=p.length;e$509<n$510;++e$509){var p$511=
triangles[2===e$509%3?e$509-2:e$509+1];if(-1===p[e$509]||-1===f[p$511])f[p$511]=e$509}p=0;for(triangles=d.length;p<triangles;++p)n[d[p]]=p;2>=d.length&&0<d.length&&(this.triangles=(new Int32Array(3)).fill(-1),this.halfedges=(new Int32Array(3)).fill(-1),this.triangles[0]=d[0],this.triangles[1]=d[1],this.triangles[2]=d[1],f[d[0]]=1,2===d.length&&(f[d[1]]=0))};Delaunay.prototype.voronoi=function(bounds){return new Voronoi(this,bounds)};Delaunay.prototype.neighbors=function $jscomp$generator$function(i){var $jscomp$generator$this=
this,$jscomp$destructuring$var79,inedges,hull,_hullIndex,halfedges,triangles,collinear,l,e0,e,p0,p;return $jscomp.generator.createGenerator($jscomp$generator$function,function($jscomp$generator$context){switch($jscomp$generator$context.nextAddress){case 1:$jscomp$destructuring$var79=$jscomp$generator$this;inedges=$jscomp$destructuring$var79.inedges;hull=$jscomp$destructuring$var79.hull;_hullIndex=$jscomp$destructuring$var79._hullIndex;halfedges=$jscomp$destructuring$var79.halfedges;triangles=$jscomp$destructuring$var79.triangles;
collinear=$jscomp$destructuring$var79.collinear;if(!collinear){$jscomp$generator$context.jumpTo(2);break}l=collinear.indexOf(i);if(!(0<l)){$jscomp$generator$context.jumpTo(3);break}return $jscomp$generator$context.yield(collinear[l-1],3);case 3:if(!(l<collinear.length-1)){$jscomp$generator$context.jumpTo(5);break}return $jscomp$generator$context.yield(collinear[l+1],5);case 5:return $jscomp$generator$context.return();case 2:e0=inedges[i];if(-1===e0)return $jscomp$generator$context.return();e=e0;p0=
...
null==context?context=new Path:void 0,hull=this.hull,points=this.points,h=2*hull[0],n=hull.length;context.moveTo(points[h],points[h+1]);for(h=1;h<n;++h){var h$514=2*hull[h];context.lineTo(points[h$514],points[h$514+1])}context.closePath();return buffer&&buffer.value()};Delaunay.prototype.hullPolygon=function(){var polygon=new Polygon;this.renderHull(polygon);return polygon.value()};Delaunay.prototype.renderTriangle=function(i,context){var buffer=null==context?context=new Path:void 0,points=this.points,
triangles=this.triangles,t0=2*triangles[i*=3],t1=2*triangles[i+1];i=2*triangles[i+2];context.moveTo(points[t0],points[t0+1]);context.lineTo(points[t1],points[t1+1]);context.lineTo(points[i],points[i+1]);context.closePath();return buffer&&buffer.value()};Delaunay.prototype.trianglePolygons=function $jscomp$generator$function(){var $jscomp$generator$this=this,$jscomp$destructuring$var85,triangles,i,n;return $jscomp.generator.createGenerator($jscomp$generator$function,function($jscomp$generator$context){1==
$jscomp$generator$context.nextAddress&&($jscomp$destructuring$var85=$jscomp$generator$this,triangles=$jscomp$destructuring$var85.triangles,i=0,n=triangles.length/3);if(3!=$jscomp$generator$context.nextAddress)return i<n?$jscomp$generator$context.yield($jscomp$generator$this.trianglePolygon(i),3):$jscomp$generator$context.jumpTo(0);++i;return $jscomp$generator$context.jumpTo(2)})};Delaunay.prototype.trianglePolygon=function(i){var polygon=new Polygon;this.renderTriangle(i,polygon);return polygon.value()};
