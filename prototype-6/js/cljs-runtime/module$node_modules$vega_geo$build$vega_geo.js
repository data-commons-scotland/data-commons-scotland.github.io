...
_.smooth),tz=_.thresholds||levels(pulse,field,_),as=null===_.as?null:_.as||"contour",values=[];pulse.forEach(function(t){var grid=field(t),paths=contour.size([grid.width,grid.height])(grid.values,vegaUtil.isArray(tz)?tz:tz(grid.values));transformPaths(paths,grid,t,_);paths.forEach(function(p){var $jscomp$compprop15={};values.push(vegaDataflow.rederive(t,vegaDataflow.ingest(null!=as?($jscomp$compprop15[as]=p,$jscomp$compprop15):p)))})});this.value&&(out.rem=this.value);this.value=out.source=out.add=
...
vegaUtil.inherits(KDE2D,vegaDataflow.Transform,{transform:function(_,pulse){if(this.value&&!pulse.changed()&&!_.modified())return pulse.StopPropagation;var out=pulse.fork(pulse.NO_SOURCE|pulse.NO_FIELDS);pulse=pulse.materialize(pulse.SOURCE).source;pulse=partition(pulse,_.groupby);var names=(_.groupby||[]).map(vegaUtil.accessorName),kde=params$jscomp$0(density2D(),_),as=_.as||"grid",values=[];values=pulse.map(function(g){var $jscomp$compprop16={},JSCompiler_temp_const=vegaDataflow.ingest;$jscomp$compprop16=
($jscomp$compprop16[as]=kde(g,_.counts),$jscomp$compprop16);g=g.dims;for(var i=0;i<names.length;++i)$jscomp$compprop16[names[i]]=g[i];return JSCompiler_temp_const.call(vegaDataflow,$jscomp$compprop16)});this.value&&(out.rem=this.value);this.value=out.source=out.add=values;return out}});Contour.Definition={type:"Contour",metadata:{generates:!0},params:[{name:"size",type:"number",array:!0,length:2,required:!0},{name:"values",type:"number",array:!0},{name:"x",type:"field"},{name:"y",type:"field"},{name:"weight",
type:"field"},{name:"cellSize",type:"number"},{name:"bandwidth",type:"number"},{name:"count",type:"number"},{name:"nice",type:"boolean","default":!1},{name:"thresholds",type:"number",array:!0},{name:"smooth",type:"boolean","default":!0}]};vegaUtil.inherits(Contour,vegaDataflow.Transform,{transform:function(_,pulse){if(this.value&&!pulse.changed()&&!_.modified())return pulse.StopPropagation;var out=pulse.fork(pulse.NO_SOURCE|pulse.NO_FIELDS),contour=contours().smooth(!1!==_.smooth),values=_.values,
thresh=_.thresholds||quantize(_.count||10,_.nice,!!values),size=_.size;if(!values){values=pulse.materialize(pulse.SOURCE).source;_=params$jscomp$0(density2D(),_)(values,!0);var post=transform(_,_.scale||1,_.scale||1,0,0);size=[_.width,_.height];values=_.values}thresh=vegaUtil.isArray(thresh)?thresh:thresh(values);values=contour.size(size)(values,thresh);post&&values.forEach(post);this.value&&(out.rem=this.value);this.value=out.source=out.add=(values||[]).map(vegaDataflow.ingest);return out}});GeoJSON.Definition=
...
lat&&pulse.modified(vegaUtil.accessorFields(lat));if(!this.value||_)fields=pulse.SOURCE,this._features=features=[],this._points=points=[];geojson&&pulse.visit(fields,function(t){return features.push(geojson(t))});lon&&lat&&(pulse.visit(fields,function(t){var x=lon(t);t=lat(t);null!=x&&null!=t&&(x=+x)===x&&(t=+t)===t&&points.push([x,t])}),features=features.concat({type:"Feature",geometry:{type:"MultiPoint",coordinates:points}}));this.value={type:"FeatureCollection",features:features}}});GeoPath.Definition=
