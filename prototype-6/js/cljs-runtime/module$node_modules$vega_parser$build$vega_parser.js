...
scope.config,style=config.style,axis=config.axis,band="band"===scope.scaleType(_$jscomp$5.scale)&&config.axisBand,orient=_$jscomp$5.orient;if(isSignal(orient)){var xyKeys=keys([config.axisX,config.axisY]),orientKeys=keys([config.axisTop,config.axisBottom,config.axisLeft,config.axisRight]);var xy={};for(var $jscomp$inline_4600=$jscomp.makeIterator(xyKeys),$jscomp$key$key=$jscomp$inline_4600.next();!$jscomp$key$key.done;$jscomp$key$key=$jscomp$inline_4600.next()){var key=$jscomp$key$key.value;xy[key]=
ifX(orient,fallback(key,config.axisX,axis,style),fallback(key,config.axisY,axis,style))}var or={};var $jscomp$inline_4602=$jscomp.makeIterator(orientKeys);for($jscomp$key$key=$jscomp$inline_4602.next();!$jscomp$key$key.done;$jscomp$key$key=$jscomp$inline_4602.next()){key=$jscomp$key$key.value;var JSCompiler_temp_const=or,JSCompiler_temp_const$jscomp$0=key,$orient=orient.signal,t=fallback(key,config.axisTop,axis,style),b=fallback(key,config.axisBottom,axis,style),l=fallback(key,config.axisLeft,axis,
...
function arrayLambda(array){for(var n=array.length,code="[",i=0;i<n;++i){var value$527=array[i];code+=(0<i?",":"")+(vegaUtil.isObject(value$527)?value$527.signal||propertyLambda(value$527):vegaUtil.stringValue(value$527))}return code+"]"}function objectLambda(obj){var code="{",i=0,key;for(key in obj){var value=obj[key];code+=(1<++i?",":"")+vegaUtil.stringValue(key)+":"+(vegaUtil.isObject(value)?value.signal||propertyLambda(value):vegaUtil.stringValue(value))}return code+"}"}function defaults(){return{description:"Vega visualization",
