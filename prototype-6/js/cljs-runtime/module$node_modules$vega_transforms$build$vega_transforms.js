...
counts,countOnly){measures=measures.map(function(m){return compileMeasures(m,m.field)});var cell={num:0,agg:null,store:!1,count:counts};if(!countOnly)for(var n$jscomp$0=measures.length,a=cell.agg=Array(n$jscomp$0),i$jscomp$0=0;i$jscomp$0<n$jscomp$0;++i$jscomp$0)a[i$jscomp$0]=new measures[i$jscomp$0](cell);if(cell.store)var store=cell.data=new TupleStore;cell.add=function(t){cell.num+=1;if(!countOnly){store&&store.add(t);for(var i$441=0;i$441<n$jscomp$0;++i$441)a[i$441].add(a[i$441].get(t),t)}};cell.rem=
function(t){--cell.num;if(!countOnly){store&&store.rem(t);for(var i$442=0;i$442<n$jscomp$0;++i$442)a[i$442].rem(a[i$442].get(t),t)}};cell.set=function(t){var n;store&&store.values();var i=0;for(n=counts.length;i<n;++i)t[counts[i]]=cell.num;if(!countOnly)for(i=0,n=a.length;i<n;++i)a[i].set(t)};cell.init=function(){cell.num=0;store&&store.reset();for(var i$443=0;i$443<n$jscomp$0;++i$443)a[i$443].init()};return cell}function Window(params){vegaDataflow.Transform.call(this,{},params);this._mlen=0;this._mods=
...
!1,field=_.field;source=_.step||vegaUtil.span(vegaUtil.extent(source,field))/30;var sort=vegaDataflow.stableCompare(function(a,b){return field(a)-field(b)});_=_.as||"bin";for(var n=groups.length,min=Infinity,max=-Infinity,i=0,j;i<n;++i){var g=groups[i].sort(sort);j=-1;for(var $jscomp$iter$110=$jscomp.makeIterator(vegaStatistics.dotbin(g,source,smooth,field)),$jscomp$key$v=$jscomp$iter$110.next();!$jscomp$key$v.done;$jscomp$key$v=$jscomp$iter$110.next())$jscomp$key$v=$jscomp$key$v.value,$jscomp$key$v<
...
d3Array.range(step/2,1-1E-14,step),n=p.length;pulse.forEach(function(g){for(var q=vegaStatistics.quantiles(g,p),i=0;i<n;++i){for(var t={},i$440=0;i$440<names.length;++i$440)t[names[i$440]]=g.dims[i$440];t[as[0]]=p[i];t[as[1]]=q[i];values.push(vegaDataflow.ingest(t))}});this.value&&(out.rem=this.value);this.value=out.add=out.source=values;return out}});vegaUtil.inherits(Relay,vegaDataflow.Transform,{transform:function(_,pulse){if(this.value)var lut=this.value;else{var out=pulse=pulse.addAll();lut=
