...
d3Array){function numbers(values,valueof){var index,$jscomp$iter$107,$jscomp$key$value,value$432,$jscomp$iter$106,value;return $jscomp.generator.createGenerator(numbers,function($jscomp$generator$context){switch($jscomp$generator$context.nextAddress){case 1:if(null==valueof){$jscomp$generator$context.jumpTo(2);break}index=-1;$jscomp$iter$107=$jscomp.makeIterator(values);$jscomp$key$value=$jscomp$iter$107.next();case 4:if($jscomp$key$value.done){$jscomp$generator$context.jumpTo(0);break}value$432=
$jscomp$key$value.value;value$432=valueof(value$432,++index,values);if(!(null!=value$432&&""!==value$432&&(value$432=+value$432)>=value$432)){$jscomp$generator$context.jumpTo(5);break}return $jscomp$generator$context.yield(value$432,5);case 5:$jscomp$key$value=$jscomp$iter$107.next();$jscomp$generator$context.jumpTo(4);break;case 2:$jscomp$iter$106=$jscomp.makeIterator(values),$jscomp$key$value=$jscomp$iter$106.next();case 9:if($jscomp$key$value.done){$jscomp$generator$context.jumpTo(0);break}value=
$jscomp$key$value.value;if(!(null!=value&&""!==value&&(value=+value)>=value)){$jscomp$generator$context.jumpTo(10);break}return $jscomp$generator$context.yield(value,10);case 10:$jscomp$key$value=$jscomp$iter$106.next(),$jscomp$generator$context.jumpTo(9)}})}function quantiles(array,p,f){var values=Float64Array.from(numbers(array,f));values.sort(d3Array.ascending);return p.map(function(_){return d3Array.quantileSorted(values,_)})}function quartiles(array,f){return quantiles(array,[.25,.5,.75],f)}
function estimateBandwidth(array,f){var n=array.length,d=d3Array.deviation(array,f);array=quartiles(array,f);return 1.06*(Math.min(d,(array[2]-array[0])/1.34)||d||Math.abs(array[0])||1)*Math.pow(n,-.2)}function smoothing(v,thresh){for(var n=v.length,a=0,b=1,c,d;v[a]===v[b];)++b;for(;b<n;){for(c=b+1;v[b]===v[c];)++c;if(v[b]-v[b-1]<thresh){for(d=b+(a+c-b-b>>1);d<b;)v[d++]=v[b];for(;d>b;)v[d--]=v[a]}a=b;b=c}return v}function sampleNormal(mean,stdev){mean=mean||0;stdev=null==stdev?1:stdev;if(nextSample===
nextSample){var x=nextSample;nextSample=NaN}else{do{x=2*exports.random()-1;var y=2*exports.random()-1;var rds=x*x+y*y}while(0===rds||1<rds);rds=Math.sqrt(-2*Math.log(rds)/rds);x*=rds;nextSample=y*rds}return mean+x*stdev}function densityNormal(value,mean,stdev){stdev=null==stdev?1:stdev;value=(value-(mean||0))/stdev;return Math.exp(-.5*value*value)/(stdev*SQRT2PI)}function cumulativeNormal(value,mean,stdev){value=(value-(mean||0))/(null==stdev?1:stdev);mean=Math.abs(value);37<mean?mean=0:(stdev=Math.exp(-mean*
mean/2),mean=7.07106781186547>mean?stdev*((((((.0352624965998911*mean+.700383064443688)*mean+6.37396220353165)*mean+33.912866078383)*mean+112.079291497871)*mean+221.213596169931)*mean+220.206867912376)/(((((((.0883883476483184*mean+1.75566716318264)*mean+16.064177579207)*mean+86.7807322029461)*mean+296.564248779674)*mean+637.333633378831)*mean+793.826512519948)*mean+440.413735824752):stdev/(mean+1/(mean+2/(mean+3/(mean+4/(mean+.65)))))/2.506628274631);return 0<value?1-mean:mean}function quantileNormal(p,
mean,stdev){if(0>p||1<p)return NaN;p=2*p-1;var w=-Math.log((1-p)*(1+p));6.25>w?(w-=3.125,w=1.6536545626831027+(.24015818242558962+(-.006033670871430149+(-7.40702534166267E-4+(1.8673420803405714E-4+(-1.3882523362786469E-5+(-1.3654692000834679E-6+(4.2347877827932404E-7+(-2.9070369957882005E-8+(-4.112633980346984E-9+(1.0512122733215323E-9+(-5.415412054294628E-11+(-1.2975133253453532E-11+(2.6335093153082323E-12+(-8.151934197605472E-14+(-4.054566272975207E-14+(6.637638134358324E-15+(2.0972767875968562E-17+
...
y){var X=0,Y=0,XY=0,X2=0,n=0;visitPoints(data,x$jscomp$0,y,function(dx,dy){++n;X+=(dx-X)/n;Y+=(dy-Y)/n;XY+=(dx*dy-XY)/n;X2+=(dx*dx-X2)/n});var coef=ols(X,Y,XY,X2),predict=function(x){return coef[0]+coef[1]*x};return{coef:coef,predict:predict,rSquared:rSquared(data,x$jscomp$0,y,Y,predict)}}function quad(data,x$jscomp$0,y){var $jscomp$destructuring$var38=$jscomp.makeIterator(points(data,x$jscomp$0,y)),xv=$jscomp$destructuring$var38.next().value,yv=$jscomp$destructuring$var38.next().value,ux=$jscomp$destructuring$var38.next().value,
uy=$jscomp$destructuring$var38.next().value,n=xv.length,X3=$jscomp$destructuring$var38=0,X4=0,XY=0,X2Y=0,i;for(i=0;i<n;){var dx=xv[i];var dy=yv[i++];var x2=dx*dx;$jscomp$destructuring$var38+=(x2-$jscomp$destructuring$var38)/i;X3+=(x2*dx-X3)/i;X4+=(x2*x2-X4)/i;XY+=(dx*dy-XY)/i;X2Y+=(x2*dy-X2Y)/i}xv=X4-$jscomp$destructuring$var38*$jscomp$destructuring$var38;yv=$jscomp$destructuring$var38*xv-X3*X3;var a=(X2Y*$jscomp$destructuring$var38-XY*X3)/yv,b=(XY*xv-X2Y*X3)/yv,c=-a*$jscomp$destructuring$var38;$jscomp$destructuring$var38=
function(x){x-=ux;return a*x*x+b*x+c+uy};return{coef:[c-b*ux+a*ux*ux+uy,b-2*a*ux,a],predict:$jscomp$destructuring$var38,rSquared:rSquared(data,x$jscomp$0,y,uy,$jscomp$destructuring$var38)}}function uncenter(k,a,x,y){var z=Array(k),i;for(i=0;i<k;++i)z[i]=0;for(i=k-1;0<=i;--i){var v=a[i];var c=1;z[i]+=v;for(k=1;k<=i;++k)c*=(i+1-k)/k,z[i-k]+=v*Math.pow(x,k)*c}z[0]+=y;return z}function gaussianElimination(matrix){var n=matrix.length-1,coef=[],i,j,k;for(i=0;i<n;++i){var r=i;for(j=i+1;j<n;++j)Math.abs(matrix[i][j])>
...
b)},pdf:function(value){return densityUniform(value,a,b)},cdf:function(value){return cumulativeUniform(value,a,b)},icdf:function(p){return quantileUniform(p,a,b)}};null==max&&(max=null==min?1:min,min=0);return dist.min(min).max(max)};exports.regressionExp=function(data,x$jscomp$0,y){var $jscomp$destructuring$var36=$jscomp.makeIterator(points(data,x$jscomp$0,y)),xv=$jscomp$destructuring$var36.next().value;$jscomp$destructuring$var36.next();var ux=$jscomp$destructuring$var36.next().value;$jscomp$destructuring$var36=
$jscomp$destructuring$var36.next().value;var YL=0,XY=0,XYL=0,X2Y=0,n=0,dx,ly,xy;visitPoints(data,x$jscomp$0,y,function(_,dy){dx=xv[n++];ly=Math.log(dy);xy=dx*dy;YL+=(dy*ly-YL)/n;XY+=(xy-XY)/n;XYL+=(xy*ly-XYL)/n;X2Y+=(dx*xy-X2Y)/n});var $jscomp$destructuring$var37=$jscomp.makeIterator(ols(XY/$jscomp$destructuring$var36,YL/$jscomp$destructuring$var36,XYL/$jscomp$destructuring$var36,X2Y/$jscomp$destructuring$var36)),c0=$jscomp$destructuring$var37.next().value,c1=$jscomp$destructuring$var37.next().value;
$jscomp$destructuring$var37=function(x){return Math.exp(c0+c1*(x-ux))};return{coef:[Math.exp(c0-c1*ux),c1],predict:$jscomp$destructuring$var37,rSquared:rSquared(data,x$jscomp$0,y,$jscomp$destructuring$var36,$jscomp$destructuring$var37)}};exports.regressionLinear=linear;exports.regressionLoess=function(data,x$jscomp$0,y,bandwidth){y=$jscomp.makeIterator(points(data,x$jscomp$0,y,!0));data=y.next().value;var yv=y.next().value;x$jscomp$0=y.next().value;y=y.next().value;var n=data.length,bw=Math.max(2,
~~(bandwidth*n));bandwidth=new Float64Array(n);for(var residuals=new Float64Array(n),robustWeights=(new Float64Array(n)).fill(1),iter=-1;2>=++iter;){for(var interval=[0,bw-1],i=0;i<n;++i){for(var dx=data[i],i0=interval[0],i1=interval[1],W=0,X=0,Y=0,XY=0,X2=0,denom=1/Math.abs(data[dx-data[i0]>data[i1]-dx?i0:i1]-dx||1);i0<=i1;++i0){var xk=data[i0],yk=yv[i0],x=Math.abs(dx-xk)*denom,w=(x=1-x*x*x)*x*x*robustWeights[i0];x=xk*w;W+=w;X+=x;Y+=yk*w;XY+=yk*x;X2+=xk*x}W=$jscomp.makeIterator(ols(X/W,Y/W,XY/W,
X2/W));i1=W.next().value;W=W.next().value;bandwidth[i]=i1+W*dx;residuals[i]=Math.abs(yv[i]-bandwidth[i]);dx=i+1;i1=data[dx];W=interval[0];X=interval[1]+1;if(!(X>=data.length))for(;dx>W&&data[X]-i1<=i1-data[W];)interval[0]=++W,interval[1]=X,++X}if(2===iter)break;interval=d3Array.median(residuals);if(1E-12>Math.abs(interval))break;i=0;for(i1=void 0;i<n;++i)dx=residuals[i]/(6*interval),robustWeights[i]=1<=dx?1E-12:(i1=1-dx*dx)*i1}yv=data.length;n=[];residuals=bw=0;for(robustWeights=[];bw<yv;++bw)iter=
data[bw]+x$jscomp$0,robustWeights[0]===iter?robustWeights[1]+=(bandwidth[bw]-robustWeights[1])/++residuals:(residuals=0,robustWeights[1]+=y,robustWeights=[iter,bandwidth[bw]],n.push(robustWeights));robustWeights[1]+=y;return n};exports.regressionLog=function(data,x$jscomp$0,y){var X=0,Y=0,XY=0,X2=0,n=0;visitPoints(data,x$jscomp$0,y,function(dx,dy){++n;dx=Math.log(dx);X+=(dx-X)/n;Y+=(dy-Y)/n;XY+=(dx*dy-XY)/n;X2+=(dx*dx-X2)/n});var coef=ols(X,Y,XY,X2),predict=function(x){return coef[0]+coef[1]*Math.log(x)};
return{coef:coef,predict:predict,rSquared:rSquared(data,x$jscomp$0,y,Y,predict)}};exports.regressionPoly=function(data,x$jscomp$0,y$jscomp$0,order){if(1===order)return linear(data,x$jscomp$0,y$jscomp$0);if(2===order)return quad(data,x$jscomp$0,y$jscomp$0);var $jscomp$destructuring$var39=$jscomp.makeIterator(points(data,x$jscomp$0,y$jscomp$0)),xv=$jscomp$destructuring$var39.next().value,yv=$jscomp$destructuring$var39.next().value,ux=$jscomp$destructuring$var39.next().value,uy=$jscomp$destructuring$var39.next().value;
$jscomp$destructuring$var39=xv.length;var lhs=[],rhs=[],k=order+1,i,l,v;for(i=0;i<k;++i){for(v=l=0;l<$jscomp$destructuring$var39;++l)v+=Math.pow(xv[l],i)*yv[l];lhs.push(v);var c=new Float64Array(k);for(order=0;order<k;++order){for(v=l=0;l<$jscomp$destructuring$var39;++l)v+=Math.pow(xv[l],i+order);c[order]=v}rhs.push(c)}rhs.push(lhs);var coef=gaussianElimination(rhs);xv=function(x){x-=ux;var y=uy+coef[0]+coef[1]*x+coef[2]*x*x;for(i=3;i<k;++i)y+=coef[i]*Math.pow(x,i);return y};return{coef:uncenter(k,
coef,-ux,uy),predict:xv,rSquared:rSquared(data,x$jscomp$0,y$jscomp$0,uy,xv)}};exports.regressionPow=function(data,x$jscomp$0,y){var X=0,Y=0,XY=0,X2=0,YS=0,n=0;visitPoints(data,x$jscomp$0,y,function(dx,dy){dx=Math.log(dx);var ly=Math.log(dy);++n;X+=(dx-X)/n;Y+=(ly-Y)/n;XY+=(dx*ly-XY)/n;X2+=(dx*dx-X2)/n;YS+=(dy-YS)/n});var coef=ols(X,Y,XY,X2),predict=function(x){return coef[0]*Math.pow(x,coef[1])};coef[0]=Math.exp(coef[0]);return{coef:coef,predict:predict,rSquared:rSquared(data,x$jscomp$0,y,YS,predict)}};
exports.regressionQuad=quad;exports.sampleCurve=function(f,extent,minSteps,maxSteps){minSteps=minSteps||25;maxSteps=Math.max(minSteps,maxSteps||200);var point=function(x){return[x,f(x)]},minX=extent[0],maxX=extent[1],span=maxX-minX,stop=span/maxSteps;extent=[point(minX)];var next=[];if(minSteps===maxSteps){for(stop=1;stop<maxSteps;++stop)extent.push(point(minX+stop/minSteps*span));extent.push(point(maxX));return extent}next.push(point(maxX));for(maxSteps=minSteps;0<--maxSteps;)next.push(point(minX+
maxSteps/minSteps*span));minSteps=extent[0];for(minX=next[next.length-1];minX;)maxSteps=point((minSteps[0]+minX[0])/2),maxSteps[0]-minSteps[0]>=stop&&Math.abs(Math.atan2(minX[1]-minSteps[1],minX[0]-minSteps[0])-Math.atan2(maxSteps[1]-minSteps[1],maxSteps[0]-minSteps[0]))>MIN_RADIANS?next.push(maxSteps):(minSteps=minX,extent.push(minX),next.pop()),minX=next[next.length-1];return extent};exports.sampleLogNormal=sampleLogNormal;exports.sampleNormal=sampleNormal;exports.sampleUniform=sampleUniform;exports.setRandom=
function(r){exports.random=r};Object.defineProperty(exports,"__esModule",{value:!0})})}
